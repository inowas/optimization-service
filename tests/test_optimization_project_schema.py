...
OPTIMIZATION_PROJECT_SCHEMA = PurePosixPath("https://schema.inowas.com/optimization/optimization_project.json")
OPTIMIZATION_PROJECT_DATA = Path(__file__).resolve().parent / "testing_data" / \
                            "rio_primero-example-optimization-start-request.json"

with urlopen(str(OPTIMIZATION_PROJECT_SCHEMA)) as f:
...
def test_optimization_project_schema():
    ...
    path_resolver = RefResolver(str(OPTIMIZATION_PROJECT_SCHEMA.parent), referrer=test_schema)
